...
/>
                ...
                <div className="absolute top-0 rounded-t-lg inset-0 bg-gradient-to-br from-wave1 to-wave2 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <p className="text-white text-center font-bold text-[50px]">{project.type}</p>
                </div>
              </div>
              ...
              {/* Hover Card */}
              <div
  className="absolute left-[50%] lg:left-[100%] transform -translate-x-1/2 top-52 mt-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-50 bg-secondary shadow-xl rounded-lg p-6 border border-primary backdrop-blur-md"
  style={{
    width: "90%",
    background: "rgba(173, 216, 230, 0.6)", // Light blue semi-transparent
                  ...
                  {project.items.map((item, idx) => (
                    <li key={idx}>
                      <a
                        ...
                        style={{
                          background: "linear-gradient(135deg, #00509e, #003f7f)", // Matching button gradient
                          boxShadow:
